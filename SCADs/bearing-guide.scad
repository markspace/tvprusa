...
bearing_inner = bearing_diameter + 1;
...
inner();
translate([ bearing_inner + 2 * wall_ascent + 2, 0, 0 ]) outer();
...
module outer()
{
	difference()
	{
		cylinder( r = bearing_inner / 2 + 2 * single_layer_width + tolerance, h = height );
		translate([ 0, 0, -1 ])
		cylinder( r = bearing_inner / 2 + single_layer_width + tolerance, h = height + 2 );
	}
