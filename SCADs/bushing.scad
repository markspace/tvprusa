...
module bushing(stiff=true, length=11)
{
	difference()
	{
		union()
...
module z_bushings()
{
	...
	union()
	{
		translate([0,9.5,0])vertical_bushing(true,13);
...
module linear_holder_base(length, lip=false)
{
	difference()
	{
		union()
...
module y_linear_bearings()
{
	difference()
	{
		...
		translate(v=[14,0,0])polyhole(m3_diameter, 10);
		//hack
		translate(v=[0,14.25,4.5]) rotate(a=[90,0,0]) translate(v=[0,9.5,0]) rotate(a=[0,0,90]) translate(v=[0,0,lm8uu_length/2+0.5]) ziptie();
...
module z_linear_bearings()
{
	translate(v=[0,9.5,0]) rotate(a=[0,0,90])
	{
		difference()
		{
			linear_holder_base(length=65, lip=true);
