...
module spring_arc(arm_len, r, th, w)
{
  ...
  union() {
    ...
    difference() {
     ...
     translate([0,0,-w/2])polyhole(d=2*or, h=w);

      translate([-or,0,0])
          cube([2*or, 2*or+2, w+2],center=true);

      //cylinder(r=r, h=w+10, center=true, $fn=20);
     translate([0,0,-(w+10)/2])polyhole(d=2*r, h=w+10);
...
module spring_square(fid=3, th=2, folds=8, lx=20, lz=20,drill=8)
{
  ...
  difference() {
    ...
    rotate([90,0,0])
     ...
     translate([0,0,-(ly+10)/2]) polyhole(d=drill, h=ly+10);
  }
}
...
translate([0,0,lz/2])
spring_square(fid=fid, th=th, folds=folds, lx=lx, lz=lz, drill=drill);
