...
gear1_setscrew_d         = 3.2;
gear1_setscrew_r          = gear1_setscrew_d/2;
gear1_captive_nut_d = 6.5;
...
gear2_bolt_hex_d       = 15; //Diameter from point to point (not flat to flat)
...
module gearsbyteethanddistance(t1=13,t2=51, d=60, teethtwist=1, which=1)
{
	...
	if(which == 1)
	{
		...
		difference()
		{
			...
			union()
			{
				...
				translate([0,0,(gear_h/2) + AT]) rotate([180,0,0]) 
					gear(	twist = -g1twist, 
						number_of_teeth=t1, 
						circular_pitch=cp, 
			...
			translate([(g1p_r)/2 + 1, 0, gear_h+gear_shaft_h-gear1_setscrew_offset]) 
				union() {
	...
	else
	{
		...
		difference()
		{
			...
			//hex bolt shaft
			translate([0,0,-AT]) cylinder(r=gear2_bolt_hex_r, h=gear2_bolt_sink, $fn=6);

			// sunken face
			translate([0, 0, gear_h / 4])
				difference() {
					cylinder(r = g2p_r - (cp / 45), h=gear_h, center=true);
					cylinder(r = gear2_shaft_outer_r, h=gear_h + TT, center=true);
				}

			// motor mount access screw
			for(i = [0:7]) {
//				translate([motor_mount_access_radius * cos((i * 45) + 22.5), motor_mount_access_radius * sin((i * 45) + 22.5), gear_h / 2])
				rotate([0, 0, (45 * i) + 22.5])
					translate([motor_mount_access_radius, 0, gear_h / 2])
					//cylinder(r=3, h=(gear_h * 2), center=true);
					rotate([0, 270, 90])
...
translate([-10, 45, 0])
	render()
		translate([-g1p_r,0,0]) rotate([0,0,($t*360/gear1_teeth)]) {
			intersection() {
				union() {
					translate([0, 0, 0]) #cylinder(r1=g1p_r, r2=g1p_r + (cp / 180), h=(cp / 180));
					translate([0, 0, (cp / 180) - 0.01]) #cylinder(r=g1p_r + (cp / 180), h=10.02 - (cp / 180) - (cp / 180));
					translate([0, 0, 10 - (cp / 180)]) #cylinder(r2=g1p_r, r1=g1p_r + (cp / 180), h=(cp / 180));
					translate([0, 0, 10]) cylinder(r1=g1p_r, r2=g1p_r + (cp / 180) + 0.1, h=(cp / 180) + 0.2);
					translate([0, 0, 10 + (cp / 180)]) cylinder(r=g1p_r + (cp / 180) + 1, h = 11);
				}
